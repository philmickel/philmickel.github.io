// -------------------------------------------------------------------
// Initialize openSimplex = openSimplexNoise(random seed)
...
const arcs01 = n => array(n).map((_, i) => (n <= 1 ? 0 : i / (n - 1)));

function thetaPhiArray(resolution = 1000) {
	phiArray = arcs01(resolution);
	thetaArray = arcs01(resolution);
	newArray = []
	for (let i = 0; i < phiArray.length; i++) {
		for (let i = 0; i < thetaArray.length; i++) {
			newArray.push([phiArray[i], thetaArray[i]]);
		}
	}
	return newArray;
}

openSimplex = openSimplexNoise(0);

const scribble = (resolution = 1000) => {
    return thetaPhiArray(resolution).map(t => {
        const phi = t[0] * Math.PI * 2;
		const theta = t[1] * Math.PI * 2;
        const freq = 13.0;
        const amplitude = 450;
        const x = Math.sin(phi) * Math.sin(theta) * freq;
        const y = Math.cos(theta) * freq;
		const z = Math.cos(phi)*Math.sin(theta) * freq;
        return [
            amplitude * openSimplex.noise4D(x, y, z, -15), 1.5*amplitude * openSimplex.noise4D(x, y, z, 10), amplitude * openSimplex.noise4D(x, y, z, 113)
        ];
    });
};

const pointArray = scribble(2000);

// Drawing lines in p5:
// line(x1, y1, z1, x2, y2, z2);
// Draws line from (x1, y1, z1) to (x2, y2, z2)
// Line width can be changed with strokeWeight() function. strokeWeight(1) is default
// strokeWeight should be called before the drawing of the line



let fps = 288;
let angle = (Math.PI/15/60);
let yRotationMatrix = [Math.cos(angle), 0, Math.sin(angle), 0, 1, 0, -Math.sin(angle), 0, Math.cos(angle)];

function matrixMultiply3D(lineVector, matrix){
	let [x1, y1, z1, x2, y2, z2] = lineVector;
	let [a, b, c, d, e, f, g, h, i] = matrix;
	return [a*x1 + d*y1 + g*z1, b*x1 + e*y1 + h*z1, c*x1 + f*y1 + i*z1, a*x2 + d*y2 + g*z2, b*x2 + e*y2 + h*z2, c*x2 + f*y2 + i*z2];
}
...
function draw() {
	if (obj.linePoints.length < FRAMES_UNTIL_LAG) {
		background(0);
		console.log(frameCount);
		[x1, y1, z1] = pointArray[frameCount];
		[x2, y2, z2] = pointArray[frameCount + 1];
		obj.linePoints.push([x1, y1, z1, x2, y2, z2]);	
		for (i = 0; i < obj.linePoints.length; i++) {
			let lineVec = obj.linePoints[i];
			obj.linePoints[i] = matrixMultiply3D(obj.linePoints[i], yRotationMatrix);
			[x1, y1, z1, x2, y2, z2] = obj.linePoints[i];
			line(x1, y1, z1, x2, y2, z2);
	}
  } else {
	  resetSketch();
